...
<body>
    ...
    // Ambil kolom jenis dari tabel informasi
    $sqlJenis = "DESCRIBE informasi jenis";
    ...
    //Menampilkan data dari informasi
    $sql = "SELECT informasi.id AS informasi_id, 
        ...
        ORDER BY informasi.date DESC, informasi.id DESC";
    
    $dataInfo = $conn->prepare($sql);
    $dataInfo->execute();
    $resultInfo = $dataInfo->get_result();

    //mengelompokan informasi berdasarkan jenisnya
    $info_jenis = [];
        ...
        <div class="main p-3">
            <main>
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-body">
                        <a class="btn btn-primary mb-4" href="create.php" style="color: white; width: 155px;"><i class="lni lni-plus"></i> Informasi Baru</a>
                            <form method="GET">
                                <div class="d-flex align-items-center gap-2">
                                    <p class="mb-0">Anda sedang menampilkan data informasi:</p>
                                        ...
                                        foreach ($info_jenis as $jenis => $info) {
                                            ...
                                            foreach ($info as $row) { ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="btn btn-sm btn-info buttons" href="show.php?id=<?= htmlspecialchars($row['informasi_id']); ?>"><i class="bx bx-show"></i></a>
                                                        <a class="btn btn-sm btn-warning buttons" href="edit.php?id=<?= htmlspecialchars($row['informasi_id']); ?>"><i class="lni lni-pencil-1"></i></a>
                                                        <a onclick="return confirm('Apakah anda yakin?')" class="btn btn-sm btn-danger buttons" href="delete.php?id=<?= htmlspecialchars($row['informasi_id']); ?>"><i class="lni lni-trash-3"></i></a>
                                                        <a class="btn btn-sm btn-primary buttons" href="../../cetak_detailNews.php?id=<?= htmlspecialchars($row['informasi_id']); ?>"><i class="lni lni-printer"></i></a>
                                                    </td>
                                                </tr>
                                            <?php }
                                        }
